...
const EpubBridge = {
  ...
  loadChapter: async function (groupHref) {
    ...
    try {
      const promises = group.spineHrefs.map((href) =>
        this._loadSingleSpineItem(href),
      );
  ...
  findPhraseIndex: function (wordsArray, phrase) {
    ...
    for (let i = 0; i < wordsArray.length; i++) {
      ...
      const currentBookWord = wordObj.text
        .toLowerCase()
        .replace(/[^\wáéíóúñü]/g, '');
      ...
      if (currentBookWord === targetTokens[0]) {
        ...
        while (tokenIdx < targetTokens.length) {
          ...
          const nextBookWord = nextWordObj.text
            .toLowerCase()
            .replace(/[^\wáéíóúñü]/g, '');
  ...
  getNextChapter: function () {
    const idx = this.getChapterIndexByHref(this.currentChapterHref);
    if (idx !== -1 && idx < this.chapters.length - 1)
      return this.chapters[idx + 1].href;
    return null;
  },
};
