...
const btnLibraryFromControls = document.getElementById(
  'btnLibraryFromControls',
);
...
async function renderLibraryList() {
  ...
  if (books.length === 0) {
    libraryList.innerHTML =
      '<div class="empty-lib-msg">No books yet. Upload one!</div>';
    return;
  }
...
tabBtns.forEach((btn) => {
  btn.addEventListener('click', () => {
    ...
    document
      .getElementById(`tab-content-${btn.dataset.target}`)
...
function skipWords(direction) {
  ...
  renderWord(words[currentIndex], wordOutput);
  showToast(
    direction === 'left' ? `⏪ -${jumpSize}` : `⏩ +${jumpSize}`,
    toast,
  );
  ...
  if (isContextOpen) {
    ...
    words.forEach((wordObj, index) => {
      if (wordObj.type === 'break') {
        contextOverlay.appendChild(document.createElement('div')).className =
          'ctx-break';
      } else {
