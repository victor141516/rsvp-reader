...
function parseHTMLToRSVP(htmlString) {
  ...
  function walk(node, style) {
    if (node.nodeType === Node.TEXT_NODE) {
      const rawParts = node.textContent
        .split(/\s+/)
        .filter((w) => w.length > 0);
      ...
      if (
        isBlock &&
        words.length > 0 &&
        words[words.length - 1].type !== 'break'
      ) {
...
function renderWord(wordObj, outputElement) {
  ...
  const isBreak =
    (wordObj && wordObj.type === 'break') || wordText === PARAGRAPH_TOKEN;
  ...
  const regex =
    /^([^\wáéíóúÁÉÍÓÚñÑüÜ]*)([\wáéíóúÁÉÍÓÚñÑüÜ\-\']+)([^\wáéíóúÁÉÍÓÚñÑüÜ]*)$/;
